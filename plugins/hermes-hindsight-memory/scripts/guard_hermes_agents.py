...
AGENTS = Path.home() / ".codex" / "HERMES_AGENTS.md"
BACKUP = AGENTS.with_name(AGENTS.name + ".bak")
LOG = Path.home() / "plugins" / "hermes_agents_guard.log"
...
def main():
    ...
    if not agents_ok and backup_ok:
        ...
        log(f"restored HERMES_AGENTS.md from backup; size={AGENTS.stat().st_size}")
        return

    if agents_ok and not backup_ok:
        BACKUP.write_text(AGENTS.read_text(encoding="utf-8"), encoding="utf-8")
        log(f"created backup from HERMES_AGENTS.md; size={BACKUP.stat().st_size}")
        return

    if agents_ok and backup_ok:
        if AGENTS.stat().st_mtime > BACKUP.stat().st_mtime:
            BACKUP.write_text(AGENTS.read_text(encoding="utf-8"), encoding="utf-8")
            log(f"refreshed backup from newer HERMES_AGENTS.md; size={BACKUP.stat().st_size}")
        else:
            log("ok")
        return

    log("missing HERMES_AGENTS.md and backup; manual restore required")
