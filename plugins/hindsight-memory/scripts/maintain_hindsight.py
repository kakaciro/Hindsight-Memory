...
FALLBACK = Path(os.environ.get("HINDSIGHT_FALLBACK_PATH") or ROOT.parent / "hindsight_fallback.json")
LOG = ROOT.parent / "hindsight_maintenance.log"
AGENTS = Path(os.environ.get("AGENTS_MD_PATH") or Path.home() / ".codex" / "AGENTS.md")
AGENTS_BACKUP = AGENTS.with_name(AGENTS.name + ".bak")
...
def dedupe_agents_rules():
    if (not AGENTS.exists() or AGENTS.stat().st_size == 0) and AGENTS_BACKUP.exists() and AGENTS_BACKUP.stat().st_size > 0:
        AGENTS.write_text(AGENTS_BACKUP.read_text(encoding="utf-8"), encoding="utf-8")
        restored = True
    else:
        restored = False
    ...
    if AGENTS.exists() and AGENTS.stat().st_size > 0:
        AGENTS_BACKUP.write_text(AGENTS.read_text(encoding="utf-8"), encoding="utf-8")
    return f"agents_rules={len(keep)} removed={removed} restored={str(restored).lower()}"
