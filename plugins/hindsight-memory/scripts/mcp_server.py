...
BANK_ID = os.environ.get("HINDSIGHT_BANK_ID", "codex")
...
FALLBACK_PATH = os.environ.get("HINDSIGHT_FALLBACK_PATH") or os.path.join(PLUGIN_ROOT, "hindsight_fallback.json")
MAINTENANCE_LOG = os.path.join(PLUGIN_ROOT, "hindsight_maintenance.log")
AGENTS_PATH = os.environ.get("AGENTS_MD_PATH") or os.path.join(os.path.expanduser("~"), ".codex", "AGENTS.md")
...
def handle_list_tools():
    return {
        "tools": [
            {
                "name": "recall_codex_memory",
                "description": "Recall Codex memories from Hindsight DB with fine-grained local fallback search.",
                "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}}, "required": ["query"]},
            },
            {
                "name": "retain_codex_memory",
...
def handle_call_tool(name, arguments):
    if name == "recall_codex_memory":
        return handle_recall(arguments.get("query", ""))
    if name == "retain_codex_memory":
        return handle_retain(arguments.get("fact", ""))
    if name == "record_codex_error":
        return handle_record_error(arguments)
    if name == "maintain_codex_memory":
        return {"content": [{"type": "text", "text": maintenance_report()}]}
    return {"content": [{"type": "text", "text": f"Unknown tool: {name}"}], "isError": True}


def handle_list_prompts():
    return {"prompts": [{"name": "hindsight-system-booster", "description": "Inject prioritized Hindsight rules and observations."}]}


def handle_get_prompt(name):
    if name != "hindsight-system-booster":
    ...
    if not block.strip():
        block = "None."
    system_instruction = (
        "You are connected to Codex Hindsight Memory.\n"
    ...
    for line in sys.stdin:
        ...
        try:
            ...
            if method == "initialize":
                result = {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}, "prompts": {}}, "serverInfo": {"name": "hindsight-codex-mcp", "version": "3.0.0"}}
